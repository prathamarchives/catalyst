...
def cmd_init(args):
    ...
    (dest / "README.md").write_text(
        ...
        "Fill it via the extract step (copy-paste compose) or onboarding.\n", encoding="utf-8")
    _emit({"ok": True, "brain": f"outputs/{paths.slug(args.name)}"}, args.json,
          f"created brain at {dest}\nnext: extract context, then `catalyst context {args.name} \"<task>\"`")
...
def main(argv=None) -> int:
    ...
    {"init": cmd_init, "status": cmd_status, "route": cmd_route, "context": cmd_context,
