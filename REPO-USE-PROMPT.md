...
Interview me in small rounds, create files under outputs/<name>/, write a creative-identity skill, run a before/after proof, then ask for feedback and update the system so it compounds session to session.
```
...
```txt
This repo is an agent protocol: read README.md and AGENTS.md, then run the creative-identity protocol on me.
